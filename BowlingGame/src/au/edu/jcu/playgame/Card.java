			...
			if(enteredScore.contentEquals(acceptableScoreEntries[i])) {
				if (matchRound == 10) {
					if (scores.size() > 1 && playerTurn == 2) {
						...
						if (spare == 10) {
							enteredScore = "/";
							scores.get(playerBowlTurn - 1).removeLastScore();
						}
					} else {
						if (!enteredScore.contentEquals("X")) {
							lastScore = Integer.parseInt(enteredScore);	
						}
					}
					scores.get(playerBowlTurn - 1).addScore(enteredScore);
					if (playerTurn == 2 && ((!enteredScore.contains("X")) || (!enteredScore.contains("/")))) {
						turnOver = true;
						playerTurn = 1;
		...
		protected void onDraw(Canvas canvas) {
			...
			for (int i = 0; i < scores.size(); i++) {
				...
				for (int j = 0; j < tempScores.size(); j++) {
					String printedScore = tempScores.get(j);
					if (printedScore.contains("X") || printedScore.contains("/")) {
						xPosition += 31;
					}
					canvas.drawText(printedScore , xPosition, yPosition, paint);
					xPosition += 31;
				}
				yPosition += 63;
			}
